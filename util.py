...
def loadData(f_name):
    ...
    return data[:-2], label[
                      2:]  # Removing first two and last two so each X[i] tries to predict Y[i+2] (i've used i+2 and not to i+1 to force it to predict the future (O) )
...
def getCurrentData(label=False):
    keys = ["price_usd", "24h_volume_usd", "market_cap_usd", "available_supply", "total_supply", "percent_change_1h",
